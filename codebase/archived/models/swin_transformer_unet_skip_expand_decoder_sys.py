...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        # Rearrange dimensions manually (b h w (p1 p2 c) -> b (h p1) (w p2) c)
        p1, p2 = 2, 2
        C_new = C // (p1 * p2)
        H_new, W_new = H * p1, W * p2
        x = x.permute(0, 3, 1, 2)  # (B, C, H, W)
        x = x.reshape(B, C_new, H_new, W_new)  # (B, C // 4, H * p1, W * p2)
        x = x.permute(0, 2, 3, 1).contiguous()  # (B, H * p1, W * p2, C // 4)
        x = x.view(B, -1, C_new)  # Flatten spatial dimensions back
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, C)  # Reshape to (B, H, W, C)
        # Rearrange dimensions manually (b h w (p1 p2 c) -> b (h p1) (w p2) c)
        C_new = C // (self.dim_scale ** 2)
        H_new, W_new = H * self.dim_scale, W * self.dim_scale
        x = x.permute(0, 3, 1, 2)  # (B, C, H, W)
        x = x.reshape(B, C_new, H_new, W_new)  # (B, C // 16, H * dim_scale, W * dim_scale)
        x = x.permute(0, 2, 3, 1).contiguous()  # (B, H * dim_scale, W * dim_scale, C // 16)
        x = x.view(B, -1, self.output_dim)  # Flatten spatial dimensions back
